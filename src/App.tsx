...
const App: React.FC = () => {
    ...
    await performTransaction((roomData) => {
      ...
      if (!handOver) {
        ...
        if (roomData.scores[handWinnerTeamId] >= 7) {
          ...
          const TELEGRAM_BOT_TOKEN = "8506797614:AAFS88cCTcGx6hpMt1gc1mUMHciM2Qgqm6k"; 

          if (tgChatId) {
              const winningTeamName = `تیم ${handWinnerTeamId}`;
              const losingTeamId = handWinnerTeamId === 1 ? 2 : 1;
              const resultText = `${roomData.scores[1]} - ${roomData.scores[2]}`;
              
              // Get names
              const winners = roomData.players.filter(p => p && p.teamId === handWinnerTeamId).map(p => p!.name).join("، ");
              const losers = roomData.players.filter(p => p && p.teamId === losingTeamId).map(p => p!.name).join("، ");

              const message = `🃏 **پایان بازی حکم حقیقت**\n\n🏆 **برنده:** ${winningTeamName} (${winners})\n💀 **بازنده:** تیم ${losingTeamId} (${losers})\n\n📊 **نتیجه:** ${resultText}\n\n👻 ثبت شده توسط روح حقیقت`;
  ...
  useEffect(() => {
    ...
    // --- Telegram Check ---
    // Check for "tgWebAppStartParam" (from BotFather) OR "startapp" OR "chat_id"
    const tgParam = params.get("tgWebAppStartParam") || params.get("startapp") || params.get("chat_id");
    if (tgParam) {
        localStorage.setItem("telegram_chat_id", tgParam);
        console.log("Detected Telegram Chat ID:", tgParam);
    }
    // ----------------------
